...
6. **Verify the write before reporting it.** After calling Write/Edit on `CHANGELOG.md`, read it back and confirm the content is actually there - report only what you just confirmed on disk, never what you composed in your own reasoning but didn't verify was saved.
7. **Stop at the draft.** Once `CHANGELOG.md` is written and the bump is recommended, tell the user exactly which `pnpm release:*` command matches your recommendation - don't run it.
